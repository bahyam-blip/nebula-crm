...
async function main() {
  ...
  const start = await jfetch('/v1/studio/build', {
    method: 'POST', token,
    body: { title: 'Kettle Theory', kind: 'landing', brief, wait: false },
  });
  ok(start.status === 202 && start.json?.ok === true && /^b_[a-z0-9]+$/.test(start.json?.job_id || ''), `live run started in-job (${start.json?.job_id})`, JSON.stringify(start.json).slice(0, 200));
  const jobId = start.json?.job_id;
  if (!jobId) { console.log('LIVE RUN FAILED TO START — aborting'); process.exit(1); }

  // Poll: the trace must GROW while the run executes — that is the whole
  // point (the user watches the real team, not a paced ticker).
  let firstCount = -1, lastCount = 0, grew = false, sawReflector = false, status = 'running';
  let finalDoc = null;
  const t0 = Date.now();
  for (let i = 0; i < 90; i++) {
    await new Promise((r) => setTimeout(r, i < 2 ? 2000 : 3000));
    const st = await jfetch(`/v1/studio/run?id=${jobId}`, { token });
    if (st.status !== 200) continue; // transient
    const n = (st.json?.trace || []).length;
    if (n > 0 && firstCount === -1) firstCount = n;
    if (n > lastCount) {
      if (lastCount > 0 && n > lastCount) grew = true;
      lastCount = n;
      console.log(`  · poll ${i + 1}: status=${st.json.status} trace=${n} rows`);
    }
  ...
  ok(status === 'done', `run reached done in ${secs}s (status=${status})`, finalDoc?.error || '');
  ok(grew === true, `trace streamed LIVE (grew from ${firstCount} to ${lastCount} rows across polls)`);
  ...
  ok(result.artifact_id && result.url, 'result payload carries the artifact + URL', JSON.stringify(result).slice(0, 160));
  ok(result.builder === 'ai', 'builder=ai — engineers hand-coded the page', result.builder);
  ok(result.team_summary?.ai_calls >= 9, `result team_summary counts the AI calls (${result.team_summary?.ai_calls})`);
  ...
  const site = await fetch(result.url, { redirect: 'follow' });
  ...
  const stranger = await jfetch(`/v1/studio/run?id=${jobId}`); // no token
  ...
  const site2 = await fetch(ref.json.url, { redirect: 'follow' });
