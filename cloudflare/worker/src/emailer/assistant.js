...
export const TOOLS = {
  ...
  build_website: { tier: 'write', roles: WRITE_ROLES, spec: 'BUILD AND HOST a complete website or mini web app {title, brief, kind?: landing|promo|event|portfolio|webapp|report, style?, cta_text?, cta_url?} — the agent RESEARCHES the market live, plans the art direction and page architecture, then HAND-CODES each section (bespoke HTML+CSS+motion, no templates), reviews its own code and hosts it. Returns a PUBLIC URL + build stages. Use for landing pages, offer pages, event invites, portfolios, product showcases, market-research one-pagers, or small interactive web apps' },
