...
export const TOOLS = {
  ...
  build_website: { tier: 'write', roles: WRITE_ROLES, spec: 'BUILD AND HOST a complete website or mini web app {title, brief, kind?: landing|promo|event|portfolio|webapp|report, style?, cta_text?, cta_url?} — a MULTI-AGENT TEAM builds it: the Lead plans the run, a Researcher scans the live web, an Art Director designs the system, a Copywriter + Copy Chief write and tighten the words, an Architect plans the sections, Engineers hand-code every section (bespoke HTML+CSS+motion, no templates), a QA Director reviews the code with a rework loop, and the Builder hosts it. Returns a PUBLIC URL + the full agent-team trace. Use for landing pages, offer pages, event invites, portfolios, product showcases, market-research one-pagers, or small interactive web apps' },
  refine_site: { tier: 'write', roles: WRITE_ROLES, spec: 'UPDATE an already-built site with a change request {artifact_id, instruction} — e.g. "make the headline punchier", "change accent to green", "add a pricing FAQ". The agent re-codes the affected page on the SAME public URL as a new version (previous versions stay accessible with ?v=N)' },
